// atitikmuo butu: localStorage.getItem("people") === nul)
// is radziu kai pirma karta atidaroma pusvalis tikria ar yra anksciau buve jei ne sukuria tuscia stringini masyva
if (!localStorage.getItem("people")) {
	//	localStorage.setItem("people",[]); - taiip gaunama klaida, nes priimama tik strong reiksme
...
// pabega numeracija, todel kai suvedi du yrasus abieju nr yra 1, tam su ifu tikrinam ar yra is viso uzsetinta nors viena reiksme, jei nera tai tada priskyriam viena
if (!localStorage.getItem("currentNumeration"))
	localStorage.setItem("currentNumeration", "1");
// gaunama local storage esama dabartine numeracija, kadangi gaunam visad stringa, tai + pavercia skaiciumi
// prie apsirasant su localStorage, reiksme buvo: let currentNumeration = 1
...
addingButtonElement.addEventListener("click", () => {
	...
	//Numeracija padideja
	currentNumeration++;
	// numeracija padideja ir local storage, todel reikalinga sita salyga, currentNumeration - yra skaicius, todel ""+ pavercia i stringu
...
document
	...
	.addEventListener("click", (event) => {
		event.preventDefault();//jei jo nebutu uzsireloadina puuslapis
		const foundIndex = people.findIndex(
			(val) => val.number === +idInputUpdate.value
		);

		if (foundIndex === -1) { //findIndex grazina -1, jei indekso neranda
			alert("Žmogaus su tokiu numeriu nėra");
			return;
		}
		...
		people[foundIndex] = {
			number: person.number, //numeriukas nekeiciamas, todel taip aprasyta
